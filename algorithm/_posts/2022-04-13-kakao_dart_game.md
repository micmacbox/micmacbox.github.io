...
function solution(dartResult) {
    var answer = 0;
    var arr = dartResult.match(/\d\D|\d{2}\D|\*|\#/g);
    answer = arr.reduce((acc,_,i)=>{

        let num = /\d{1,2}/g.exec(_)*1;
        let bonus = /\D/g.exec(_)[0];
        let char = /\*|\#/g.exec(_);
        ...
        if(char!=null && char[0]==='*'){
            if(acc.length>1) acc[acc.length-2]*=2;
    ...
    answer = answer.reduce((acc,_)=>{return acc+=_},0);
    return answer;
}
```
