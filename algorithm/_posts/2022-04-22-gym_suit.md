...
date: 2022-04-22 20:30 +0800
# toc: true
---

[K번째수](https://programmers.co.kr/learn/courses/30/lessons/42862)
...
function solution(n, lost, reserve) {
    ...
    for(let i=0; i<realLost.length; i++){
        let idx = realLost[i]-1;

        if(idx < 1 || realReserve.indexOf(idx)===-1){
            idx = realLost[i]+1;
        }

        if(idx<=n && realReserve.indexOf(idx) != -1 ){
            let p = realReserve.splice(realReserve.indexOf(idx), 1);
            if(p.length) answer++;
        }
    }
